...
class ExtendibleHashTable : public HashTable<K, V> {
 ...
 private:
   ...
   */
  auto RedistributeBucket(std::shared_ptr<ExtendibleHashTable<K, V>::Bucket> &src_bucket,std::shared_ptr<ExtendibleHashTable<K, V>::Bucket> &dst_bucket,int index) -> void;
